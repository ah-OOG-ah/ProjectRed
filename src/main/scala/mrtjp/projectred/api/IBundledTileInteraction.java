...
/**
 * Class used instead of implementing IBundledTile, where it would be very, very inconvienient (such as when PR adds
 * this functionality to other mods). This class externally handles passing signal info from the tile that *should* have
 * implemented IBundledTile to the requesting device.
 *
 * This should be a standalone class that you have to create on the side, you probably dont want this to be implemented
 * on your tile.
 *
...
public interface IBundledTileInteraction {
    ...
    /**
     * Checks if this interaction can run at the given position. (ie, if the position contains the tile this interaction
     * is meant for)
     *
