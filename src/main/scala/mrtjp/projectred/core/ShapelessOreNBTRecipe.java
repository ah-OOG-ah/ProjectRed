    ...
    public ItemStack getCraftingResult(InventoryCrafting inv) {
        ...
        if (keepNBT) {
            ...
            for (int i = 0; i < 9; i++) {
                ...
                if (slot != null) if (slot.hasTagCompound()) {
                    oldItemWithNBT = slot;
                    break;
                }
            }
            if (oldItemWithNBT != null) out.setTagCompound(oldItemWithNBT.getTagCompound());
        }
        return out;
    }

    private boolean checkItemEquals(ItemStack target, ItemStack input) {
        if (target.getItem() == input.getItem() && (target.getItemDamage() == OreDictionary.WILDCARD_VALUE
                || target.getItemDamage() == input.getItemDamage())) {
            if (checkNBT) return target.getTagCompound().equals(input.getTagCompound());
            return true;
        }
        return false;
    }
