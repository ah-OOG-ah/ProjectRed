...
trait TFaceAcquisitions extends TAcquisitionsCommons with TFaceOrient {
  ...
  def posOfCorner(r: Int) =
    new BlockCoord(tile).offset(absoluteDir(r)).offset(side)
  override def posOfStraight(r: Int) =
    new BlockCoord(tile).offset(absoluteDir(r))
...
trait TConnectableCommons extends TMultiPart with IConnectable {
  ...
  /** Sets of defs that are common in all subtypes. dir is rotation for face,
    * absDir for center implementations
    */

  /** Should always return true if this is a logic part for example, where mask
    * is always open, because edges are are all the way on the side, so strips
    * cant block conns.
    */
  ...
  /** Used to determine which conns can be made outside. Implementations include
    * 1) for parts that can make external conns, check if strips, etc. is
    * blocking. 2) for parts that cannot, always false 3) for parts that take up
    * the entire face, always true (i.e. logic tiles)
    *
    * @param dir
    *   rot, or absDir (face vs center parts)
    * @return
    *   true if a connection is possible
    */
  ...
  /** Recalculates connections that can be made to other parts outside of this
    ...
    *
    * @return
    *   true if external connections should be recalculated
    */
  ...
  /** Recalculates connections to blocks outside this sapce
    *
    * @return
    *   true if a new connection was added or one was removed
    */
...
trait TFaceConnectable extends TConnectableCommons with TFaceAcquisitions {
  ...
  /** Split into 4 nybbles: (from lowest) 0 = Corner connections (this wire
    * should connect around a corner to something external) 1 = Straight
    * connections (this wire should connect to something external) 2 = Internal
    * connections (this wire should connect to something internal) 3 = Open
    * sides (this wire is not blocked by a cover/edge part and *could* connect
    * through side) bit 16 = connection to the centerpart 5 = Render corner
    * connections. Like corner connections but set to low if the other wire part
  ...
  /** If this is a wire, should return true if this wire is smaller than that
    * wire. This is used for corner rendering. (This renders to that, not that
    * to this). Always false if this is not a wire.
    *
    * @param part
    *   The part to connect to
    * @return
    *   true if this should render instead of that
    */
  ...
  def discoverCorner(r: Int): Int = {
    if (outsideCornerEdgeOpen(r)) {
      getCorner(r) match {
        case c: IConnectable =>
          if (
            (c.canConnectCorner(rotFromCorner(r)) || canConnectCorner(r)) &&
  ...
  override def discoverStraight(r: Int) = getStraight(r) match {
    case c: IConnectable =>
      canConnectPart(c, r) && c.connectStraight(this, rotFromStraight(r), -1)
  ...
  override def discoverInternal(r: Int) = {
    if (tile.partMap(PartMap.edgeBetween(absoluteDir(r), side)) == null)
      getInternal(r) match {
        case c: IConnectable =>
          canConnectPart(c, r) && c.connectInternal(this, rotFromInternal(r))
  ...
  override def updateExternalConns() = {
    ...
    if (newConn != (connMap & 0xf000ff)) {
      val diff =
        connMap ^ newConn // corners need to be notified, because normal block updates wont touch them
...
trait TCenterConnectable extends TConnectableCommons with TCenterAcquisitions {
  ...
  /** Split into 3 sections: (from lowest) 0x3F = Straight conns to outside
    * 0xFC0 = Internal conns to face (mixed with above for client, see
    * clientConnMap) 0x3F000 = External open connections (this wire is not
    * blocked by a cover part and *could* connect through side)
    */
  override var connMap = 0
  ...
  override def discoverStraight(s: Int) = getStraight(s) match {
    case c: IConnectable =>
      canConnectPart(c, s) && c.connectStraight(this, s ^ 1, -1)
  ...
  override def updateExternalConns() = {
    var newConn = 0
    for (s <- 0 until 6)
      if (maskOpen(s)) if (discoverStraight(s)) newConn |= 1 << s
