...
object PRLib {
  ...
  def getMultipartTile(w: IBlockAccess, bc: BlockCoord): TileMultipart =
    getMultipartTile(w, bc.x, bc.y, bc.z)
  def getMultipartTile(w: IBlockAccess, x: Int, y: Int, z: Int) =
    w.getTileEntity(x, y, z) match {
      case t: TileMultipart => t
      case _                => null
    }

  def getMultiPart(w: IBlockAccess, bc: BlockCoord, part: Int): TMultiPart =
    getMultiPart(w, bc.x, bc.y, bc.z, part)
