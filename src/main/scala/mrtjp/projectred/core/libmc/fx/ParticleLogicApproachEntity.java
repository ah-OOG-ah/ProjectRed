...
public class ParticleLogicApproachEntity extends ParticleLogic {
    ...
    public void doUpdate() {
        ...
        } else if (target instanceof EntityItem) deltaY = posY - target.posY;
        else deltaY = (target.boundingBox.minY + target.boundingBox.maxY) / 2.0D - posY;
        ...
        if (distanceToTarget <= targetDistance * targetDistance) finishLogic();
        else particle.setPosition(posX, posY, posZ);
    }

    @Override
    public ParticleLogic clone() {
        return new ParticleLogicApproachEntity(target, approachSpeed, targetDistance).setFinal(finalLogic)
