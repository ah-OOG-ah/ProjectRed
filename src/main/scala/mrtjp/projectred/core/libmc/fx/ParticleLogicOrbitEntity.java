...
public final class ParticleLogicOrbitEntity extends ParticleLogic {
    ...
    private void generateNewTargetY() {
        if (target != null) targetY = new Random().nextDouble() * target.height;
        else targetY = 0.0D;
    }
    ...
    public void doUpdate() {
        if (firstTick) curYOffset = particle.posY - (target.posY + target.getEyeHeight());
        ...
        if (Math.abs(targetY - curYOffset) < 0.1D) generateNewTargetY();

        double posX = target.posX + Math.cos(orbitAngle) * targetDistance;
        double posZ = target.posZ + Math.sin(orbitAngle) * targetDistance;

        if (targetY < curYOffset) curYOffset -= orbitSpeed / 4.0D;
        else if (targetY > curYOffset) curYOffset += orbitSpeed / 4.0D;
        ...
        if (!ignoreYCoordinate) if (orbitY != -512.0D) posY = target.posY + target.getEyeHeight() + orbitY;
        else {
            int offset = 0;
    ...
    public ParticleLogic clone() {
        ParticleLogicOrbitEntity clone = new ParticleLogicOrbitEntity(target, orbitSpeed)
