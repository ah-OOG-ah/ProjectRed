...
public final class ParticleLogicOrbitPoint extends ParticleLogic {
    ...
    private void generateNewTargetY() {
        if (target != null) targetY = new Random().nextDouble() * 2.0D;
        else targetY = 0.0D;
    }

    private void generateNewDistance() {
        if (target != null) targetDistance = new Random().nextDouble() * 2.0D;
        else targetDistance = 0.0D;
    }
    ...
    public void doUpdate() {
        ...
        if (!ignoreYCoord && Math.abs(particle.posY - relativeTargetY) < 0.1D) generateNewTargetY();
        ...
        if (useCurrentDistance) {
            ...
            if (shrinkingOrbit) if (targetDistance <= shrinkTargetDistance) shrinkingOrbit = false;
            ...
            } else targetDistance -= shrinkSpeed;

            posX = target.x + Math.cos(orbitAngle) * targetDistance;
            posZ = target.z + Math.sin(orbitAngle) * targetDistance;
        }

        if (!ignoreYCoord) if (particle.posY < relativeTargetY) particle.posY += 0.1D;
        else if (particle.posY > relativeTargetY) particle.posY -= 0.1D;

        if (rotateClockwise) orbitAngle += orbitSpeed;
        else orbitAngle -= orbitSpeed;
        if (orbitAngle > 360.0D) orbitAngle -= 360.0D;
        else if (orbitAngle < 0.0D) orbitAngle += 360.0D;

        if (orbitY != -512.0D) posY = target.y + orbitY;
    ...
    public ParticleLogic clone() {
        ...
        if (orbitY != -512.0D) clone.setOrbitY(orbitY);
        ...
        if (shrinkingOrbit) clone.setShrinkingOrbit(shrinkSpeed, shrinkTargetDistance);
