...
public class ParticleLogicTrail extends ParticleLogic {
    ...
    public void doUpdate() {
        ...
        if (updateTicks == ticksBetweenSpawns) {
            updateTicks = 0;
            CoreParticle c = ParticleManagement.instance
                    .spawn(particle.worldObj, particleName, particle.posX, particle.posY, particle.posZ);
            if (c != null) builder.build(c);
        }
    }

    @Override
    public ParticleLogic clone() {
        ParticleLogicTrail clone = new ParticleLogicTrail(particleName, builder)
