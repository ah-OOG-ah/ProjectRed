        ...
        ref.get match {
          ...
          case None =>
            throw new UnexpectedException(
              "Empty reference was added when building power conductor cache"
            )
        }
    }
  }
...
trait TFacePowerPart
    extends TMultiPart
    with TFaceConnectable
    with TPowerPartCommons {
        ...
        getCorner(id) match // corner
        {
          ...
          case _ =>
            WorldLib.getTileEntity(
              ...
              case p: IPowerConnectable if outsideCornerEdgeOpen(id) =>
                return p.conductor(absoluteDir(rotFromCorner(id)))
              case _ =>
            }
        }
      else if ((connMap & 0x10 << id) != 0)
        getStraight(id) match // straight
        {
          ...
          case _ =>
            WorldLib.getTileEntity(
              ...
              case p: IPowerConnectable =>
                return p.conductor(absoluteDir(rotFromStraight(id)))
              case _ =>
            }
        }
      else if ((connMap & 0x100 << id) != 0)
        getInternal(id) match // internal face
        {
...
trait TCenterPowerPart
  ...
  override def getExternalCond(id: Int): PowerConductor = {
    if (0 until 6 contains id) {
      ...
      else if ((connMap & 1 << id + 6) != 0)
        getInternal(id) match // internal
        {
          case p: IPowerConnectable => return p.conductor(id ^ 1)
          case _                    =>
        }
