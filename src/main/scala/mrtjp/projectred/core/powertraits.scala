...
trait IPowerConnectable extends IConnectable {
  ...
  /** Getter for the local conductor
    * @param dir
    *   Side of the required conductor, this is only used if the tile has
    *   multiple linked conductors (such as voltage transformers). Rotation for
    *   face parts, absDir else.
    * @return
    *   The local conductor managed by this object.
    */
  def conductor(dir: Int): PowerConductor

  /** This should reach out and grab a conductor from another TPowerConnectable
    * through the method above.
    * @param id
    *   Each conductor this can possibly grab should have a designated ID. This
    *   is used internally. Calling this method with the same ID should yield
    *   the same neighbor conductor.
    * @return
    *   The neighbor conductor that corresponds with the id. Can be NULL if that
    *   ID is not connected, etc.
    */
  def conductorOut(id: Int): PowerConductor

  /** Reference to the world. This is used for checking world time to accurately
    * calculate/distribute current.
    * @return
    *   The world this TPowerConnectable is in. Should be not null.
    */
  def world: World
}

/** Object held by conducting power tiles, self managed through
  * TPowerConnectable. This model of electrical flow loosely emulates phisics of
  * real world electrical flow in a series circuit.
  * @param parent
  *   The "actual" conductor (as in, the tile).
  * @param ids
  *   The possible connections to other conductors this can make.
  */
