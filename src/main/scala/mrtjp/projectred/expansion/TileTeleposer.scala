...
class TileTeleposer extends TileMachine with TPoweredMachine {
  ...
  def tryInfusePearlItem() {
    ...
    if (
      ei != null && ei.getEntityItem.getItem == Items.ender_pearl && ei.getEntityItem.stackSize == 1
    ) {
      ei.setDead()
  ...
  def getDestination: BlockCoord = {
    val ei = getProminentHeldItem
    if (
      ei != null && ei.getEntityItem.getItem
        .isInstanceOf[ItemInfusedEnderPearl] &&
  ...
  def doPearlBeamFX() {
    ...
    for (i <- elist.indices) {
      ...
      if (
        source.copy
          .subtract(new Vector3(ent.posX, ent.posY, ent.posZ))
          .mag() < 0.75
      )
  ...
  def doRandomSparklies() {
    if (
      isCharged && world.getTotalWorldTime % 15 == 0 && world.rand
        .nextDouble() > 0.20
    ) {
  ...
  def doTransformFX() {
    for (i <- 0 until 16) {
      ...
      p.rgb =
        new Vector3(Colors.MAGENTA.rF, Colors.MAGENTA.gF, Colors.MAGENTA.bF)
      p.scale = new Vector3(0, 0, 0)
      ...
      val a1 = sequence(
        group(
          ...
          moveTo(
            start.x + world.rand.nextDouble() - 0.5,
            start.y + world.rand.nextDouble() - 0.5,
            start.z + world.rand.nextDouble() - 0.5,
            10
          )
        ),
        sequence(
...
object TeleposedEnderPearlProperty {
  ...
  def setTeleposed(e: EntityEnderPearl) {
    e.getExtendedProperties("teleposed")
      .asInstanceOf[TeleposedEnderPearlProperty]
      .isTeleposed = true
  }
