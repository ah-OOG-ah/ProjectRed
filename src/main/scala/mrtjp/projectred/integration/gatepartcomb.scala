...
abstract class ComboGateLogic
    extends RedstoneGateLogic[ComboGatePart]
    with TSimpleRSGateLogic[ComboGatePart] {
  ...
  def cycleShape(shape: Int): Int = {
    ...
    do shape1 = ComboGateLogic.advanceDead(shape1) while (bitCount(
      shape1
...
object LightSensor extends ComboGateLogic {
  ...
  override def onTick(gate: ComboGatePart) {
    ...
    def sky = gate.world.getSavedLightValue(
      EnumSkyBlock.Sky,
      gate.x,
      gate.y,
      gate.z
    ) - gate.world.skylightSubtracted
    def block =
      gate.world.getSavedLightValue(EnumSkyBlock.Block, gate.x, gate.y, gate.z)
...
object RainSensor extends ComboGateLogic {
  ...
  override def onTick(gate: ComboGatePart) {
    ...
    val newOutput =
      if (
        gate.world.isRaining && gate.world.canBlockSeeTheSky(
          gate.x,
          gate.y + 1,
          gate.z
        )
