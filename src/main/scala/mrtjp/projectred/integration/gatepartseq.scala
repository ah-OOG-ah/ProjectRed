...
class SequentialGatePartT extends SequentialGatePart with INeighborTileChange {
  ...
  override def weakTileChanges() =
    getLogic[INeighborTileChange].weakTileChanges()
...
abstract class SequentialGateLogic(val gate: SequentialGatePart)
    extends RedstoneGateLogic[SequentialGatePart]
    with TComplexGateLogic[SequentialGatePart] {
  def tickSound() {
    if (Configurator.logicGateSounds)
      gate.world.playSoundEffect(
        gate.x + 0.5,
        gate.y + 0.5,
        gate.z + 0.5,
        "random.click",
...
class SRLatch(gate: SequentialGatePart)
    extends SequentialGateLogic(gate)
...
class ToggleLatch(gate: SequentialGatePart)
    extends SequentialGateLogic(gate)
    with TExtraStateLogic {
...
trait TTimerGateLogic extends SequentialGateLogic with ITimerGuiLogic {
  ...
  abstract override def save(tag: NBTTagCompound) {
    ...
    tag.setLong(
      "pelapsed",
      if (pointer_start < 0) pointer_start
      else gate.world.getTotalWorldTime - pointer_start
    )
  }
  ...
  abstract override def load(tag: NBTTagCompound) {
    ...
    if (pointer_start >= 0)
      pointer_start =
        MultipartSaveLoad.loadingWorld.getTotalWorldTime - pointer_start
  }
  ...
  def sendPointerUpdate() {
    gate
      .getWriteStreamOf(13)
      .writeInt(if (pointer_start < 0) -1 else pointerValue)
  }
...
class Timer(gate: SequentialGatePart)
    extends SequentialGateLogic(gate)
...
class Sequencer(gate: SequentialGatePart)
    extends SequentialGateLogic(gate)
    with ITimerGuiLogic {
  var pointer_max = 40
...
class Counter(gate: SequentialGatePart)
    extends SequentialGateLogic(gate)
...
class StateCell(gate: SequentialGatePart)
    extends SequentialGateLogic(gate)
    with TTimerGateLogic
    with TExtraStateLogic {
...
class Synchronizer(gate: SequentialGatePart)
    extends SequentialGateLogic(gate)
    with TExtraStateLogic {
...
class Comparator(gate: SequentialGatePart)
    extends SequentialGateLogic(gate)
  ...
  def calcInputA: Int = {
    ...
    if (block != null) {
      if (block.hasComparatorInputOverride)
        return block.getComparatorInputOverride(
          gate.world,
          pos.x,
          pos.y,
          pos.z,
  ...
  def calcInput =
    getAnalogInput(1) << 4 | calcInputA << 8 | getAnalogInput(3) << 12
