...
class APIImpl_Transmission extends ITransmissionAPI {
        ...
        for (r <- 0 until 4) {
          ...
          tmp.partMap(pside) match {
            case ibe: IBundledEmitter =>
              raise(ibe, Rotation.rotationTo(pside, side ^ 1))
            case _ =>
          }
        }
        tmp.partMap(6) match {
  ...
  override def registerBundledTileInteraction(
      interaction: IBundledTileInteraction
  ) {
  ...
  override def getFramedWireConnectionMask(
      world: World,
      x: Int,
      y: Int,
      z: Int
  ...
  override def containsBundledCable(
      world: World,
      x: Int,
      y: Int,
      z: Int,
