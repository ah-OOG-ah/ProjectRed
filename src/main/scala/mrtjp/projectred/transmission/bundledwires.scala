...
trait TBundledCableCommons
    ...
    with TBundledAquisitionsCommons
    with IBundledCablePart {
  ...
  override def resolveArray(part: Any, r: Int) = {
    part match {
      ...
      case b: IBundledEmitter =>
        BundledCommons.raiseSignal(tmpSignal, b.getBundledSignal(r))
      case t: TileEntity =>
        BundledCommons.raiseSignal(
          tmpSignal,
          APIImpl_Transmission.getBundledSignal(
            t.getWorldObj,
            t.xCoord,
            t.yCoord,
            t.zCoord,
  ...
  override def propagateTo(part: TMultiPart, mode: Int) = {
    def shouldPropogate(part: TMultiPart, mode: Int) = part match {
      case ins: IInsulatedRedwirePart =>
  ...
  override def getBundledSignal(side: Int) =
    if (maskConnects(side)) getBundledSignal else null
  ...
  override def test(player: EntityPlayer) = {
    if (!world.isRemote) {
      var s = ""
      for (i <- 0 until 16)
        if (getBundledSignal.apply(i) != 0) s = s + "[" + i + "]"
...
class BundledCablePart
    extends WirePart
    with TFaceBundledAquisitions
    with TBundledCableCommons {
  ...
  override def calcStraightArray(r: Int) = {
    WorldLib.getTileEntity(world, posOfStraight(r)) match {
      case ibe: IBundledEmitter =>
        resolveArray(ibe, absoluteDir(rotFromStraight(r)))
      case t: TileEntity
          if APIImpl_Transmission.isValidInteractionFor(
            world,
            t.xCoord,
            t.yCoord,
            t.zCoord
  ...
  override def discoverStraightOverride(absDir: Int) = {
    val pos = new BlockCoord(tile).offset(absDir)
    world.getTileEntity(pos.x, pos.y, pos.z) match {
      case b: IMaskedBundledTile =>
        b.canConnectBundled(absDir ^ 1) &&
        (b.getConnectionMask(absDir ^ 1) & 1 << Rotation.rotationTo(
          absDir,
          side
        )) != 0
...
class FramedBundledCablePart
    extends FramedWirePart
    with TCenterBundledAquisitions
    with TBundledCableCommons {
  override def calculateSignal = {
    tmpSignalClear()
    for (s <- 0 until 6)
