...
trait TFaceRSAcquisitions
    extends TRSAcquisitionsCommons
    with TFaceAcquisitions
    with IFaceRedstonePart {
  ...
  override def calcInternalSignal(r: Int) =
    resolveSignal(getInternal(r), rotFromInternal(r))

  def calcCenterSignal = resolveSignal(getCenter, side)

  def calcStrongSignal(r: Int) =
    RedstoneInteractions.getPowerTo(this, absoluteDir(r)) * 17
  ...
  def calcWeakSignal(r: Int) = {
    ...
    if (world.isBlockNormalCubeDefault(pos.x, pos.y, pos.z, false))
      world.getBlockPowerInput(pos.x, pos.y, pos.z) * 17
    else 0
  }
  ...
  def calcDustRedwireSignal(r: Int) = {
    ...
    if (b == Blocks.redstone_wire)
      Math.max(world.getBlockMetadata(pos.x, pos.y, pos.z) - 1, 0)
    else -1
  }
  ...
  override def propagate(prev: TMultiPart, mode: Int) {
    ...
    for (r <- 0 until 4) if ((propagationMask & 1 << r) != 0) {
      if (maskConnectsInside(r)) propagateInternal(getInternal(r), prev, mode)
        ...
        propagateExternal(getCorner(r), posOfCorner(r), prev, mode)
    }
  ...
  override def propagate(prev: TMultiPart, mode: Int) {
    ...
    for (s <- 0 until 6) if ((propagationMask & 1 << s) != 0) {
      if (maskConnectsIn(s)) propagateInternal(getInternal(s), prev, mode)
        ...
        propagateExternal(getStraight(s), posOfStraight(s), prev, mode)
    }
    propagateOther(mode)
  }
}
