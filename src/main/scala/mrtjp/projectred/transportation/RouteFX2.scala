...
object RouteFX2 {
  ...
  private def spawnType1_do(colour: Int, pipe: TNetworkPipe) {
    ...
    particle.texture =
      "projectred:textures/particles/large_colourless_bubble.png"
    particle.alpha = 0
  ...
  def sendPacket(w: World, bc: BlockCoord, id: Int, colour: Int, dir: Int) {
    val packet = new PacketCustom(
      ...
      TransportationSPH.particle_Spawn
    )
    packet.writeByte(id)
        ...
        id match {
          ...
          case 2 =>
            spawnType2_do(
              colour,
              in.readUByte(),
              pipe.asInstanceOf[TNetworkPipe]
            )
          case 3 =>
            spawnType3_do(
              colour,
...
object BeamPathFinder {
  ...
  private def getMultiPart(bc: BlockCoord) =
    PRLib.getMultiPart(pipe.world, bc, 6)

  private def traceAndVectorize(
      start: BlockCoord,
      path: Seq[Int]
  ): Seq[Vector3] = {
...
private object Node {
  ...
  def apply(bc: BlockCoord, dir: Int): Node =
    new Node(bc.copy.offset(dir), 1, dir, Seq(dir))
}
private class Node(
    val bc: BlockCoord,
    val dist: Int,
    val dir: Int,
    val path: Seq[Int] = Seq.empty
