...
class RoutedFirewallPipe extends AbstractNetPipe with TNetworkPipe {
  ...
  private def writeInfo(out: MCDataOutput) {
    out
      .writeBoolean(filtExclude)
      .writeBoolean(allowRoute)
      .writeBoolean(allowBroadcast)
      .writeBoolean(allowCrafting)
  }
  ...
  override def networkFilter =
    (if (allowRoute) 0x1 else 0) | (if (allowBroadcast) 0x2 else 0) | (if (
                                                                         allowCrafting
                                                                       ) 0x4
                                                                       else 0)
...
class ContainerFirewallPipe(pipe: RoutedFirewallPipe, player: EntityPlayer)
    extends NodeContainer {
  {
    for (
      ((x, y), i) <- GuiLib.createSlotGrid(26, 17, 4, 4, 0, 0).zipWithIndex
    ) {
