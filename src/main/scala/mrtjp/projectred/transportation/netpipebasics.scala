...
class NetworkValvePipePart
  ...
  /** 0000 EEWW SSNN UUDD
    * -> OI
    *
    * 0x0 - None 0x1 - Input only 0x2 - Output only 0x3 - Input and Output
    */
  ...
  override def activate(
    ...
    if (
      item != null && item.getItem.isInstanceOf[IScrewdriver] &&
      item.getItem.asInstanceOf[IScrewdriver].canUse(player, item) &&
      ((0 until 6) contains hit
        .asInstanceOf[ExtendedMOP]
        .data
        .asInstanceOf[Int]) && !player.isSneaking
    ) {
      if (!world.isRemote) {
