...
object TransportationCPH extends TransportationPH with IClientPacketHandler {
  ...
  private def openRequestGui(packet: PacketCustom, mc: Minecraft) {
    ...
    if (p.isInstanceOf[IWorldRequester])
      mc.displayGuiScreen(new GuiRequester(p.asInstanceOf[IWorldRequester]))
  }
}
...
object TransportationSPH extends TransportationPH with IServerPacketHandler {
  def handlePacket(
    ...
    case this.gui_Request_listRefresh =>
  ...
  private def handleFirewallAction(
      packet: PacketCustom,
      sender: EntityPlayerMP
  ) {
  ...
  private def handleRouterUtilAction(
      packet: PacketCustom,
      sender: EntityPlayerMP
  ) {
  ...
  private def handleRequestListRefresh(
      packet: PacketCustom,
      sender: EntityPlayerMP
  ) {
    val bc = packet.readCoord
    ...
    if (t.isInstanceOf[IWorldRequester])
      sendRequestList(
        t.asInstanceOf[IWorldRequester],
        sender,
        packet.readBoolean,
        packet.readBoolean
      )
  ...
  private def handleRequestAction(
      packet: PacketCustom,
      sender: EntityPlayerMP
  ) {
    val bc = packet.readCoord
  ...
  private def sendRequestList(
      requester: IWorldRequester,
      player: EntityPlayerMP,
      collectBroadcast: Boolean,
      collectCrafts: Boolean
  ) {
  ...
  private def handleRequestSubmit(
      packet: PacketCustom,
      sender: EntityPlayerMP
  ) {
    val bc = packet.readCoord
    ...
    if (t.isInstanceOf[IWorldRequester]) {
      ...
      val r = new RequestConsole(opt.result())
        .setDestination(t.asInstanceOf[IWorldRequester])
      ...
      if (r.requested > 0) {
        sender.addChatMessage(
          new ChatComponentText(
            "Successfully requested " + r.requested + " of " + s.key.getName + "."
          )
        )
        RouteFX2.spawnType1(
          RouteFX2.color_request,
          t.asInstanceOf[IWorldRouter].getContainer
        )
      } else {
        sender.addChatMessage(
          new ChatComponentText(
            "Could not request " + s.stackSize + " of " + s.key.getName + ". Missing:"
          )
        )
        for ((k, v) <- r.getMissing)
          sender.addChatMessage(new ChatComponentText(v + " of " + k.getName))
      }
  ...
  private def setChipNBT(packet: PacketCustom, player: EntityPlayerMP) {
    ...
    if (
      stackInSlot == null || !stackInSlot.getItem
        .isInstanceOf[ItemRoutingChip] || !stack.getItem
        .isInstanceOf[ItemRoutingChip]
    ) {
      player.playerNetServerHandler.kickPlayerFromServer(
        player.getDisplayName + " tried to cheat with \"RedGive\"-Exploit!"
      )
      ProjectRedCore.log.error(
        player.getDisplayName + " tried to cheat with \"RedGive\"-Exploit!"
      )
      return
    }
