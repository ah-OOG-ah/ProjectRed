...
class AbstractPipePayload(val payloadID: Int) {
  ...
  def speed_=(f: Float) {
    data = data & 0xffff00ff | ((f * 100).toInt & 0xff) << 8
  }

  def progress = ((data >> 16) & 0xff) / 100.0f
  def progress_=(f: Float) {
    data = data & 0xff00ffff | ((f * 100).toInt & 0xff) << 16
  }
  ...
  def getEntityForDrop(x: Int, y: Int, z: Int): EntityItem = {
    ...
    val item =
      new EntityItem(parent.world, deltaX, deltaY, deltaZ, payload.makeStack)
    item.motionX = 0
    item.motionY = 0
    item.motionZ = 0
    item.hoverStart = 0
...
class NetworkPayload(payloadID: Int) extends AbstractPipePayload(payloadID) {
  ...
  def refreshIP() {
    ...
    if (router == null || router.getID != destinationUUID)
      destinationIP = RouterServices.getIPforUUID(destinationUUID)
  }
}
...
class PayloadMovement[T <: AbstractPipePayload] {
  ...
  def get(id: Int): T =
    delegate.find(_.payloadID == id).getOrElse(null.asInstanceOf[T])
