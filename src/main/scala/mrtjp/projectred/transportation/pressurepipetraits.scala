...
trait TPressureSubsystem extends PayloadPipePart[PressurePayload] {
  ...
  def passToPressureDevice(r: PressurePayload) =
    WorldLib.getTileEntity(world, posOfStraight(r.output)) match {
...
class PressureTube
    extends PayloadPipePart[PressurePayload]
    with TRedstonePipe
    with TPressureTube

class ResistanceTube
    extends PayloadPipePart[PressurePayload]
    with TRedstonePipe
    with TPressureTube {
